...
func (suite *WebhookWorkerTestSuite) TestWebhookDeliveryWithoutSignature() {
	receivedSignature := ""
	...
	server := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		suite.Equal(req.URL.String(), "/webhook")
		receivedSignature = req.Header.Get("X-Signature")
		hasSignatureHeader = req.Header.Get("X-Signature") != ""
		rw.Write([]byte(`OK`))
	}))
	defer server.Close()
	...
	// Create webhook delivery without signature (nil)
	deliveryId, err := factory.WebhookDeliveryWithSignature(server.URL+"/webhook", `{"key": "value"}`, 0, nil)
	suite.NoError(err, "Failed to create webhook delivery without signature")
	...
	suite.Equal("", receivedSignature, "X-Signature header should be empty when no signature")
	suite.False(hasSignatureHeader, "X-Signature header should not be present when no signature")
	...
	suite.Equal(200, int(delivery.ResponseCode.Int64))
	suite.Equal(1, delivery.TryCount)
	suite.False(delivery.Signature.Valid, "Signature should be NULL in database")
}
