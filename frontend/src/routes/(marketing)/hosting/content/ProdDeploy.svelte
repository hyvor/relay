...
<p>
	Hyvor Relay relies on OIDC for authentication. Create an application for Hyvor Relay in your
	OIDC provider. Make sure to allow the following URLs in your OIDC provider:
</p>
...
<ul>
	...
	<li>
		...
		<code>S3_SECRET</code>, <code>S3_BUCKET</code> (optional): Email contents are stored on the local
		filesystem by default. You can connect your own S3-compatible storage platform by setting these
