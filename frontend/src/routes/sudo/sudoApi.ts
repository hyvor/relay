export interface ConsoleSudoOptions {
	endpoint: string;
	data?: Record<string, any> | FormData;
	signal?: AbortSignal;
}

interface CallOptions extends ConsoleSudoOptions {
	method: 'get' | 'post' | 'patch' | 'delete' | 'put';
}

function getSudoApi() {
	const apiBaseUrl = '/api/sudo/';
	...
	return {
		call,
		get: async <T>(opt: ConsoleSudoOptions) => call<T>({ ...opt, method: 'get' }),
		post: async <T>(opt: ConsoleSudoOptions) => call<T>({ ...opt, method: 'post' }),
		patch: async <T>(opt: ConsoleSudoOptions) => call<T>({ ...opt, method: 'patch' }),
		delete: async <T>(opt: ConsoleSudoOptions) => call<T>({ ...opt, method: 'delete' })
	};
}
